...
def main(cfg: DictConfig):
    ...
    downstream_task_name = ['agnews','dbpedia','customized_dataset','factsnet', 'sst2', 'trec']
    ...
    def constructing_dataset(total_queries, dataset_name, NN_subspace, random_seed, query_number):
        ...
        if dataset_name == 'agnews' or dataset_name == 'dbpedia' or dataset_name == 'sst2' or dataset_name == 'trec':
    ...
    for TASK_NAME in downstream_task_name:
        ...
        if TASK_NAME == 'agnews':
            ...
            constructing_dataset(total_queries, 'agnews')

        elif TASK_NAME == 'sst2':
            sst2 = load_dataset("sst2")
            sst2_train_df = pd.DataFrame(sst2['train'])
            sst2_train_df.drop(['idx'],axis=1,inplace=True)

            query_result = construct_queries(sst2_train_df, query_number, random_seed)
            query_unique_length = 2
            total_queries = pd.DataFrame(query_result.reshape(query_unique_length*query_number,2), columns=['text','label'])
            constructing_dataset(total_queries, 'sst2')
            ...
            total_queries = pd.DataFrame(query_result.reshape(query_unique_length*query_number,2), columns=['text','label'])
            constructing_dataset(total_queries, 'trec')
            ...
            for query_number in [100,50,25,5]:
                for random_seed in [5678]:
                    ...
                    constructing_dataset(total_queries, 'dbpedia', 10, random_seed,query_number)
