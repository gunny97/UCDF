...
class HateEncoder(object):
    ...
    def __init__(self, ckpt, dataset, existed):
        if existed:
            ...
            self.model = AutoModelForSequenceClassification.from_pretrained(ckpt, num_labels=6)
...
def main(TRAIN_MODE, random_state, ss_size):

    label_0_data = pd.read_csv(f"../../building_dataset/trec/QUERY_10_SS_SIZE_50/0th_label_{random_state}.csv")
    label_1_data = pd.read_csv(f"../../building_dataset/trec/QUERY_10_SS_SIZE_50/1th_label_{random_state}.csv")
    label_2_data = pd.read_csv(f"../../building_dataset/trec/QUERY_10_SS_SIZE_50/2th_label_{random_state}.csv")
    label_3_data = pd.read_csv(f"../../building_dataset/trec/QUERY_10_SS_SIZE_50/3th_label_{random_state}.csv")
    label_4_data = pd.read_csv(f"../../building_dataset/trec/QUERY_10_SS_SIZE_50/4th_label_{random_state}.csv")
    label_5_data = pd.read_csv(f"../../building_dataset/trec/QUERY_10_SS_SIZE_50/5th_label_{random_state}.csv")

    label_0_data.drop(["Unnamed: 0"],axis=1,inplace=True)
    label_1_data.drop(["Unnamed: 0"],axis=1,inplace=True)
    label_2_data.drop(["Unnamed: 0"],axis=1,inplace=True)
    label_3_data.drop(["Unnamed: 0"],axis=1,inplace=True)
    label_4_data.drop(["Unnamed: 0"],axis=1,inplace=True)
    label_5_data.drop(["Unnamed: 0"],axis=1,inplace=True)
    ...
    query_result = construct_queries(trec_train_df, 10, random_state)       

    
    label_0_data = pd.concat([label_0_data, pd.DataFrame(query_result[0])[0]],axis=0)
    label_1_data = pd.concat([label_1_data, pd.DataFrame(query_result[1])[0]],axis=0)
    label_2_data = pd.concat([label_2_data, pd.DataFrame(query_result[2])[0]],axis=0)
    label_3_data = pd.concat([label_3_data, pd.DataFrame(query_result[3])[0]],axis=0)
    label_4_data = pd.concat([label_4_data, pd.DataFrame(query_result[4])[0]],axis=0)
    label_5_data = pd.concat([label_5_data, pd.DataFrame(query_result[5])[0]],axis=0)

    label_0_data['label'] = 0
    label_1_data['label'] = 1
    label_2_data['label'] = 2
    label_3_data['label'] = 3
    label_4_data['label'] = 4
    label_5_data['label'] = 5


    label_0_data = label_0_data.drop_duplicates(subset=None, keep='first', inplace=False, ignore_index=False)
    label_1_data = label_1_data.drop_duplicates(subset=None, keep='first', inplace=False, ignore_index=False)
    label_2_data = label_2_data.drop_duplicates(subset=None, keep='first', inplace=False, ignore_index=False)
    label_3_data = label_3_data.drop_duplicates(subset=None, keep='first', inplace=False, ignore_index=False)
    label_4_data = label_4_data.drop_duplicates(subset=None, keep='first', inplace=False, ignore_index=False)
    label_5_data = label_5_data.drop_duplicates(subset=None, keep='first', inplace=False, ignore_index=False)


    dataset = pd.concat([label_0_data, label_1_data, label_2_data, label_3_data, label_4_data, label_5_data],axis=0)
    dataset = dataset.sample(frac=1).reset_index(drop=True)
    dataset = dataset.drop([0],axis=1)
    dataset = dataset.dropna()
    model = AutoModelForSequenceClassification.from_pretrained(simcse_ckpt, num_labels=6)
    ...
    if TRAIN_MODE:
        ...
        for epoch in range(num_epochs):
            ...
            if avg_vloss < best_vloss:
                ...
                model.save_pretrained(f"../../finetuning/Multi_class/ckpt/{random_state}_TREC_ss50")
        ...
        # validation
        model = AutoModelForSequenceClassification.from_pretrained(f"../../finetuning/Multi_class/ckpt/{random_state}_TREC_ss50", num_labels=6)
        model.to(device)
        pred = []
        ref = []
    ...
    trec = load_dataset("trec")
    trec_test_df = pd.DataFrame(trec['test'])

    trec_test_df.drop(['label-fine'],axis=1,inplace=True)
    trec_test_df.columns = ['label','text']    
    Test_data = trec_test_df

    pred, logit, f1_score, acc_score, rec_score, prec_score = evaluate_result(ckpt=f'../../finetuning/Multi_class/ckpt/{random_state}_TREC_ss50', dataset=Test_data, existed=False)
...
if __name__ == "__main__":
    ...
    fileHandler = logging.FileHandler('./TREC_finetuned_test_results.log')
    ...
    print(f"TREC content detection classifier is training start!!!!!!!!!!!!!!!!!")

    for ss_size in [50]:
        for random_state in [1004, 1234, 5678, 7777, 9999]:

            f1, acc, rec, prec = main(True, random_state, ss_size) 

            print(f"TREC content detection classifier Evaluation start!!!!!!!!!!!!!!!!!")
            logger.setLevel(level=logging.DEBUG)
            logger.debug(f"content_name: TREC___ss size: {ss_size} random state: {random_state}")
