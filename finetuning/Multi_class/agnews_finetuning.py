...
def main(TRAIN_MODE,random_state):
    ...
    data_path = "/home/local/anaconda3/envs/paper/DPR/AGNews_query_number_ablation_result_NEW/subspace_size/QUERY_100_SS_SIZE_10_pos"
    ...
    query_result = construct_queries(ag_news_train_df, 10,1111)           


    # total_queries = pd.read_csv("/home/local/anaconda3/envs/paper/DPR/AGNews_text_output/query/query.csv")

    # world = pd.concat([world, pd.DataFrame(total_queries.iloc[:5]['text'], columns=['text'])],axis=0)
    # sports = pd.concat([sports, pd.DataFrame(total_queries.iloc[5:10]['text'], columns=['text'])],axis=0)
    # business = pd.concat([business, pd.DataFrame(total_queries.iloc[10:15]['text'], columns=['text'])],axis=0)
    # sci_tech = pd.concat([sci_tech, pd.DataFrame(total_queries.iloc[15:]['text'], columns=['text'])],axis=0)
...
if __name__ == "__main__":
    ...
    for random_state in [1234,5678,1004,9999,7777]:
        ...
        logger.debug(f"content_name: AGNews Query 100 random state: {random_state}")
