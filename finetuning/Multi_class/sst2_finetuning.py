...
def main(TRAIN_MODE, random_state, ss_size):
    ...
    negative = pd.read_csv(f"../../building_dataset/sst2/QUERY_10_SS_SIZE_{ss_size}/negative_{random_state}.csv")
    positive = pd.read_csv(f"../../building_dataset/sst2/QUERY_10_SS_SIZE_{ss_size}/positive_{random_state}.csv")
    ...
    sst2_train.drop(['idx'],axis=1,inplace=True)

    sst2_train.columns = ['text','label']

    query_result = construct_queries(sst2_train, 10, random_state)       

    
    negative = pd.concat([negative, pd.DataFrame(query_result[0])[0]],axis=0)
    positive = pd.concat([positive, pd.DataFrame(query_result[1])[0]],axis=0)

    negative['label'] = 0
    positive['label'] = 1


    negative = negative.drop_duplicates(subset=None, keep='first', inplace=False, ignore_index=False)
    positive = positive.drop_duplicates(subset=None, keep='first', inplace=False, ignore_index=False)


    dataset = pd.concat([negative, positive],axis=0)
    ...
    if TRAIN_MODE:
        ...
        for epoch in range(num_epochs):
            ...
            if avg_vloss < best_vloss:
                ...
                model.save_pretrained(f"../../finetuning/Multi_class/ckpt/{random_state}_SS_50_SST2")
        ...
        # validation
        model = AutoModelForSequenceClassification.from_pretrained(f"../../finetuning/Multi_class/ckpt/{random_state}_SS_50_SST2", num_labels=2)
        model.to(device)
        pred = []
        ref = []
    ...
    with open(f"sst2_test.txt", "r") as f:
    ...
    Test_data = pd.concat([test_sentences, test_labels],axis=1)
    Test_data.columns = ['text','label']

    pred, logit, f1_score, acc_score, rec_score, prec_score = evaluate_result(ckpt=f'../../finetuning/Multi_class/ckpt/{random_state}_SS_50_SST2', dataset=Test_data, existed=False)
...
if __name__ == "__main__":
    ...
    fileHandler = logging.FileHandler('./SST2_finetuned_test_results.log')
    ...
    print(f"SST2 content detection classifier is training start!!!!!!!!!!!!!!!!!")

    for ss_size in [50,100]:
        for random_state in [1004, 1234, 5678, 7777, 9999]:

            f1, acc, rec, prec = main(True, random_state, ss_size) 

            print(f"SST2 content detection classifier Evaluation start!!!!!!!!!!!!!!!!!")
            logger.setLevel(level=logging.DEBUG)
            logger.debug(f"content_name: SST2___ss size: {ss_size} random state: {random_state}")
