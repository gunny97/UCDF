...
def main(TRAIN_MODE, random_state):
    ...
    query_result = construct_queries(dbpedia_train, 10, random_state)           

    # total_queries = pd.read_csv("../../building_dataset/dbpedia/query/query.csv")

    # class_0 = pd.concat([class_0, pd.DataFrame(total_queries.iloc[:5]['text'], columns=['text'])],axis=0)
    # class_1 = pd.concat([class_1, pd.DataFrame(total_queries.iloc[5:10]['text'], columns=['text'])],axis=0)
    # class_2 = pd.concat([class_2, pd.DataFrame(total_queries.iloc[10:15]['text'], columns=['text'])],axis=0)
    # class_3 = pd.concat([class_3, pd.DataFrame(total_queries.iloc[15:20]['text'], columns=['text'])],axis=0)
    # class_4 = pd.concat([class_4, pd.DataFrame(total_queries.iloc[20:25]['text'], columns=['text'])],axis=0)
    # class_5 = pd.concat([class_5, pd.DataFrame(total_queries.iloc[25:30]['text'], columns=['text'])],axis=0)
    # class_6 = pd.concat([class_6, pd.DataFrame(total_queries.iloc[30:35]['text'], columns=['text'])],axis=0)
    # class_7 = pd.concat([class_7, pd.DataFrame(total_queries.iloc[35:40]['text'], columns=['text'])],axis=0)
    # class_8 = pd.concat([class_8, pd.DataFrame(total_queries.iloc[40:45]['text'], columns=['text'])],axis=0)
    # class_9 = pd.concat([class_9, pd.DataFrame(total_queries.iloc[45:50]['text'], columns=['text'])],axis=0)
    # class_10 = pd.concat([class_10, pd.DataFrame(total_queries.iloc[50:55]['text'], columns=['text'])],axis=0)
    # class_11 = pd.concat([class_11, pd.DataFrame(total_queries.iloc[55:60]['text'], columns=['text'])],axis=0)
    # class_12 = pd.concat([class_12, pd.DataFrame(total_queries.iloc[60:65]['text'], columns=['text'])],axis=0)
    # class_13 = pd.concat([class_13, pd.DataFrame(total_queries.iloc[65:]['text'], columns=['text'])],axis=0)
