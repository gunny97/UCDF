...
def main(model, tokenizer, device, path, lang_type, query_num):
    ...
    dataloader = DataLoader(setup, batch_size= 30, shuffle=False)
    model.eval()
    ...
    translated_df.to_csv(f"data_{query_num}/bt_results/{name}.csv")
...
if __name__ == "__main__":
    ...
    for lang_type in lang_type_list:
        ...
        tokenizer.src_lang = lang_type 
        
        
        for data_type in data_path_list:
            data_path = f"data_{query_num}/bt/{data_type}_{lang_type}.csv"

            main(model, tokenizer, device, data_path, lang_type, query_num)