...
def main(TRAIN_MODE, query_set_number):

    path = f"/home/local/anaconda3/envs/paper/DPR/finetuning/augmentation/data_{query_set_number}/eda/"

    business = pd.read_csv(path + "eda_business.csv", header=None)
    world = pd.read_csv(path + "eda_world.csv", header=None)
    sports = pd.read_csv(path + "eda_sports.csv", header=None)
    sci_tech = pd.read_csv(path + "eda_sci_tech.csv", header=None)

    business.columns = ['text']
    world.columns = ['text']
    sports.columns = ['text']
    sci_tech.columns = ['text']         
    ...
    if TRAIN_MODE:
        ...
        for epoch in range(num_epochs):
            ...
            if avg_vloss < best_vloss:
                ...
                model.save_pretrained(f"/home/local/anaconda3/envs/paper/DPR/finetuning/augmentation/finetuning/ckpt/eda_version_{query_set_number}")
    ...
    # validation
    model = AutoModelForSequenceClassification.from_pretrained(f"/home/local/anaconda3/envs/paper/DPR/finetuning/augmentation/finetuning/ckpt/eda_version_{query_set_number}", num_labels=4)
    model.to(device)
    pred = []
    ref = []
    ...
    pred, logit, f1_score, acc_score, rec_score, prec_score = evaluate_result(ckpt=f'/home/local/anaconda3/envs/paper/DPR/finetuning/augmentation/finetuning/ckpt/eda_version_{query_set_number}', dataset=Test_data, existed=False)
...
if __name__ == "__main__":
    ...
    fileHandler = logging.FileHandler('./9999_EDA_BT_augmented_log.log')
    ...
    query_set_number = 5
    f1, acc, rec, prec = main(True,query_set_number) 

    logger.setLevel(level=logging.DEBUG)
    logger.debug(f"EDA augmented version {query_set_number} queries")
