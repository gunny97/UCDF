...
def main(TRAIN_MODE,query_number_set):
    ...
    path = f"/home/local/anaconda3/envs/paper/DPR/finetuning/augmentation/data_{query_number_set}/"

    business = pd.read_csv(path + "business.csv")
    world = pd.read_csv(path + "world.csv")
    sports = pd.read_csv(path + "sports.csv")
    sci_tech = pd.read_csv(path + "sci_tech.csv")
    ...
    if TRAIN_MODE:
        ...
        for epoch in range(num_epochs):
            ...
            if avg_vloss < best_vloss:
                ...
                model.save_pretrained(f"/home/local/anaconda3/envs/paper/DPR/finetuning/augmentation/finetuning/ckpt/without_da_version_{query_number_set}")
    ...
    # validation
    model = AutoModelForSequenceClassification.from_pretrained(f"/home/local/anaconda3/envs/paper/DPR/finetuning/augmentation/finetuning/ckpt/without_da_version_{query_number_set}", num_labels=4)
    model.to(device)
    pred = []
    ref = []
    ...
    pred, logit, f1_score, acc_score, rec_score, prec_score = evaluate_result(ckpt=f'/home/local/anaconda3/envs/paper/DPR/finetuning/augmentation/finetuning/ckpt/without_da_version_{query_number_set}', dataset=Test_data, existed=False)
...
if __name__ == "__main__":
    ...
    fileHandler = logging.FileHandler('./7777_EDA_BT_augmented_log.log')
    ...
    logger.debug(f"Without Data augmented version {query_number_set} queries")
