...
def main(TRAIN_MODE,query_number_set):

    path = f"/home/local/anaconda3/envs/paper/DPR/finetuning/augmentation/data_{query_number_set}/bt_english_final_result/"

    business = pd.read_csv(path + "business_bt_results.csv", sep='\t')
    world = pd.read_csv(path + "world_bt_results.csv", sep='\t')
    sports = pd.read_csv(path + "sports_bt_results.csv", sep='\t')
    sci_tech = pd.read_csv(path + "sci_tech_bt_results.csv", sep='\t')
    ...
    trainset.to_csv('TT_trainset.csv')           
    validset.to_csv('TT_validset.csv')           

    train_setup = contentDataset(file = "TT_trainset.csv",tok = tokenizer, max_len = 128)
    valid_setup = contentDataset(file = "TT_validset.csv",tok = tokenizer, max_len = 128)
    ...
    if TRAIN_MODE:
        ...
        for epoch in range(num_epochs):
            ...
            if avg_vloss < best_vloss:
                ...
                model.save_pretrained(f"/home/local/anaconda3/envs/paper/DPR/finetuning/augmentation/finetuning/ckpt/bt_version_query_{query_number_set}")
    ...
    # validation
    model = AutoModelForSequenceClassification.from_pretrained(f"/home/local/anaconda3/envs/paper/DPR/finetuning/augmentation/finetuning/ckpt/bt_version_query_{query_number_set}", num_labels=4)
    model.to(device)
    pred = []
    ref = []
    ...
    pred, logit, f1_score, acc_score, rec_score, prec_score = evaluate_result(ckpt=f'/home/local/anaconda3/envs/paper/DPR/finetuning/augmentation/finetuning/ckpt/bt_version_query_{query_number_set}', dataset=Test_data, existed=False)
...
if __name__ == "__main__":
    ...
    fileHandler = logging.FileHandler('./1234_EDA_BT_augmented_log.log')
    ...
    query_number_set = 100
    f1, acc, rec, prec = main(True, query_number_set) 

    logger.setLevel(level=logging.DEBUG)
    logger.debug(f"BT augmented version {query_number_set}")
